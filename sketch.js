...
// ------------------- DYNAMIC CIRCLE SETUP (SHIFTED UP) -------------------
function setupCircles() {
  ...
  // Shift circles higher so they don't cover text
  const verticalShift = min(width, height) * 0.15;
  const rowYTop = height / 2 - min(width, height) * 0.2 - verticalShift;
  const rowYBottom = height / 2 + min(width, height) * 0.2 - verticalShift;

  circleRadius = min(width / (topRowCount + 1), width / (bottomRowCount + 1), height * 0.12);
